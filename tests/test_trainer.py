...
class TestTrainer(unittest.TestCase):
    """
    XXX: add a trainer class (see https://github.com/tilman151/unittest_dl/blob/master/src/trainer.py)
    Test it properly
    Split this in trainer.py
    """

    def setUp(self):
        self.hp = {
            "batch_size": 32,
            # XXX: add reduced sample data in tests or dedicated dir
        ...
        # XXX: use a temporary dir (TemporaryDir)
        log_path = os.path.join("/tmp", "logs")
        os.makedirs(log_path, exist_ok=True)
        self.sw = SummaryWriter(log_path)

    def _test_train(self, device, epochs):
        # XXX: train more time on a smaller batch and check that loss go below 1
    ...
    def test_train_cuda(self):
        ...
        """
        validation_loss = self._test_train("cuda", 1)
    ...
    def test_train_mps(self):
        ...
        """
        validation_loss = self._test_train("mps", 1)
        print(validation_loss)
